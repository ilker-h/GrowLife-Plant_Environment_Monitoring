...
void AsyncSI7021::_write(uint8_t reg, bool i2cStopMessage) {
  i2cStopMessage=true; // We added this
  Wire.beginTransmission(SI7021_I2C_ADDRESS);
  Wire.write(reg);
  Wire.endTransmission(i2cStopMessage);
}

void AsyncSI7021::_writeValue(uint8_t reg, uint8_t value, bool i2cStopMessage) {
  i2cStopMessage=true; // We added this
  Wire.beginTransmission(SI7021_I2C_ADDRESS);
  Wire.write(reg);
  Wire.write(value);
